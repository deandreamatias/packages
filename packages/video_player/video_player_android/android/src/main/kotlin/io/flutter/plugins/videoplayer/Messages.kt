 ...
 * Generated class from Pigeon that represents data sent in messages.
 */
data class DurationUpdateEvent (
  /** The video duration in milliseconds. */
  val duration: Long
) : PlatformVideoEvent()
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): DurationUpdateEvent {
      val duration = pigeonVar_list[0] as Long
      return DurationUpdateEvent(duration)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    ...
    val other = other as DurationUpdateEvent
...
private open class MessagesPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      136.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          DurationUpdateEvent.fromList(it)
        }
      }
      137.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
